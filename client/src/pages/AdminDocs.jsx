...
const AUDIO_CATALOG = {
    ...
    rover_mela: [
        ...
        { group: 'Gameplay Feedback', groupIcon: '🎯', items: [
            ...
            { file: 'wrong_move.wav', label: 'Wrong Move Sound', type: 'feedback_no', objective: 'Plays when the child taps a non-adjacent or otherwise invalid cell. Gentle corrective signal, not a hazard/fail sound.', trigger: 'Invalid cell tap (not adjacent to the token current position)', screen: 'Game Screen — grid navigation', behavior: 'Soft corrective feedback — distinct from failure.wav, which covers hazard/timeout/out-of-coins fails.' },
            ...
            { file: 'failure.wav', label: 'Question Failure Sound', type: 'feedback_no', objective: 'Intended to play when a question ends in failure — hazard cell hit, timeout, or coins exhausted.', trigger: 'Question ends without reaching the end point', screen: 'Game Screen — question end', behavior: 'Referenced in source as resolveAudio(failure, failure.wav) but no failure.wav file exists in the bundled asset folder — same gap as move.wav above.' },
            { file: 'start_trial.wav', label: 'Trial Start Sound', type: 'transition', objective: 'Intended to play when a teaching question trial begins or restarts (including via Retake).', trigger: 'Teaching question trial starts, or the child uses Retake', screen: 'Game Screen — teaching question start', behavior: 'Referenced in source as resolveAudio(start_trial, start_trial.wav) but no start_trial.wav file exists in the bundled asset folder — same gap as move.wav above.' },
        ]},
        { group: 'Alerts', groupIcon: '⚡', items: [
            { file: 'timer_warning.wav', label: 'Timer Warning', type: 'alert', objective: 'Warning alert played when the countdown reaches 6 seconds remaining on a scored question. Signals urgency without causing panic.', trigger: 'Timer countdown reaches exactly 6 seconds (timeRemaining === 6)', screen: 'Game Screen — timer area', behavior: 'Urgency signal — triggers at exact 6-second mark, not a general low-time range.' },
        ]},
    ],
    auditory_dhyan: [
