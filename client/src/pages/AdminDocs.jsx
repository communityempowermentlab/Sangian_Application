...
const AUDIO_CATALOG = {
    ...
    rover_mela: [
        ...
        { group: 'Gameplay Feedback', groupIcon: '🎯', items: [
            ...
            { file: 'wrong_move.wav', label: 'Wrong Move Sound', type: 'feedback_no', objective: 'Plays when the child taps a non-adjacent or otherwise invalid cell. Gentle corrective signal, not a hazard/fail sound.', trigger: 'Invalid cell tap (not adjacent to the token current position)', screen: 'Game Screen — grid navigation', behavior: 'Soft corrective feedback — distinct from failure.wav, which covers hazard/timeout/out-of-coins fails. Do not confuse this with the Results screen\'s hazard-fail label, "Question Ended Due to Illegal Moves" — that text names the unrelated Hit Weed condition (see Technical Documentation §4), not an invalid tap.' },
            ...
            { file: 'start_trial.wav', label: 'Trial Start Sound', type: 'transition', objective: 'Intended to play via the shared startTrial() function, which begins EVERY question and trial — not just teaching ones.', trigger: 'Any question or trial starts: the very first attempt on any teaching or scored question, a teaching Retake, or a scored-question Refresh', screen: 'Game Screen — any question start', behavior: 'Referenced in source as resolveAudio(start_trial, start_trial.wav) but no start_trial.wav file exists in the bundled asset folder — same gap as move.wav above.' },
        ]},
