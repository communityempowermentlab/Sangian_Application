...
function buildGameDataFromElements(elements) {
  ...
  return {
    1: { questionOrder: 1, isSample: true,  imageCount: 10, category: 'item0', imageIds: r(getPool('item0'), 10) },
    2: { questionOrder: 2, isSample: false, imageCount: 10, category: 'item1', imageIds: r(getPool('item1'), 10) },
    3: { questionOrder: 3, isSample: false, imageCount: 10, category: 'item2', imageIds: r(getPool('item2'), 10) },
    4: { questionOrder: 4, isSample: false, imageCount: 10, category: 'item3', imageIds: r(getPool('item3'), 10) },
    5: { questionOrder: 5, isSample: false, imageCount: 10, category: 'item4', imageIds: r(getPool('item4'), 10) },
    6: { questionOrder: 6, isSample: false, imageCount: 10, category: 'item5', imageIds: r(getPool('item5'), 10) },
    7: { questionOrder: 7, isSample: false, imageCount: 10, category: 'item6', imageIds: r(getPool('item6'), 10) },
    8: { questionOrder: 8, isSample: false, imageCount: 10, category: 'item7', imageIds: r(getPool('item7'), 10) },
    9: { questionOrder: 9, isSample: false, imageCount: 10, category: 'item8', imageIds: r(getPool('item8'), 10) },
  };
}
...
const HerPherGameV2 = () => {
  ...
  const { getAudioUrl, ready: audioReady } = useTestAudio('working_memory_herpher_v2');
  ...
  useEffect(() => {
    ...
    axios.get(`${API_URL}/public/element-positions/working_memory_herpher_v2`)
      .then(({ data }) => {
  ...
  const generateShuffledPositions = useCallback((qData, qIdx, canvasDimensions) => {
    let positions;
    const customLayout = elementPositionLayouts[qIdx - 1];
  ...
  const finishQuestion = useCallback((resp, sel, qData) => {
    ...
    let newConsecutiveZeros = consecutiveZeros;
    if (currentAttempt === 2 && !qData.isSample) {
      setTotalScore(s => s + questionScore);
      if (questionScore === 0) {
        newConsecutiveZeros += 1;
      } else {
        newConsecutiveZeros = 0;
      }
  ...
  }, [currentQuestion, currentAttempt, questionTime, consecutiveZeros]); // eslint-disable-line
  ...
  const generateAndUploadPDF = async () => {
    ...
    try {
      ...
      // Clone into a clean wrapper on document.body — .hp-container is
      // `width/height:100%` + overflow:hidden + backdrop-filter, so capturing
      // the live element in place clips it instead of its full content size.
  ...
  const getSplashImage = () => {
    if (!allElements || allElements.length === 0) return "/assets/images/her_pher_v2/her_pher_v2.jpg";
    const SERVER_BASE = API_URL.replace(/\/api$/, '');
    ...
    return "/assets/images/her_pher_v2/her_pher_v2.jpg";
  };
      ...
      <div className="hp-container">
        ...
        <header className="hp-topbar">
          <div className="hp-brand">
            ...
            {showGameIcon && <img src="/assets/images/her_pher_v2/her_pher_v2.jpg" alt="Her Pher" className="hp-test-logo" />}
            {showGameName && <span className="hp-test-title">{t('home.games.herpher.title')}{t('common.version2')}</span>}
          </div>
              ...
              <div className="hp-card hp-result-card" style={{ padding: 30 }} id="dashboard-capture-area">
                ...
                <div className="hp-score-top">
                  <div className="hp-score-dial-container">
                    ...
                    <div className="hp-score-dial-small">/ 16</div>
                  </div>
                  ...
                  <div className="hp-metric-grid">
                    ...
                    <div className="hp-metric-box">
                      <label>% Accuracy <span className="kpi-formula-icon" data-tooltip="Correct Answers ÷ Total Score (16) × 100">ⓘ</span></label>
                      <div className="metric-val">{((totalScore / 16) * 100).toFixed(1)}%</div>
                      <div className="metric-sub">{totalScore} / 16</div>
                    </div>
                    <div className="hp-metric-box">
