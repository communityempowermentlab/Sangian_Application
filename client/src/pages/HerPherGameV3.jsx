...
import './HerPherGameV3.css';

// ─── Constants ─────────────────────────────────────────────────────────────────
const GAME_NAME     = 'working_memory_herpher_v3';
const AUDIO_PATH    = '/assets/audios/her_pher_v3';
const IMAGE_PATH    = '/assets/images/her_pher_v3/items';
...
// value = { maxCorrect: pts, maxCorrect-1: pts, ... }. Total across the 8
// scored questions maxes out at 25.
const SCORING_RULES = {
  2: { 7: 2, 6: 1 },
  3: { 8: 2, 7: 1 },
  4: { 9: 3, 8: 2, 7: 1 },
...
const HerPherGameV3 = () => {
  ...
  const { getAudioUrl, ready: audioReady } = useTestAudio('working_memory_herpher_v3');
  ...
  useEffect(() => {
    ...
    axios.get(`${API_URL}/public/element-positions/working_memory_herpher_v3`)
      .then(({ data }) => {
  ...
  const startNewGame = async () => {
    try {
      const res = await axios.post(`${API_URL}/games/sessions/start`, {
        ...
        game_name: GAME_NAME,
        total_questions: 8, // 8 scored questions — practice round is excluded from progress
      });
  ...
  const completeGame = useCallback((autoStopped = false) => {
    ...
    axios.put(`${API_URL}/games/sessions/update/${sessId}`, {
      score: ts,
      // Count of scored questions actually completed (practice excluded) — 8 on
  ...
  const getSplashImage = () => {
    if (!allElements || allElements.length === 0) return "/assets/images/her_pher_v3/her_pher_v3.jpg";
    const SERVER_BASE = API_URL.replace(/\/api$/, '');
    ...
    return "/assets/images/her_pher_v3/her_pher_v3.jpg";
  };
      ...
      <div className="hp-container">
        ...
        <header className="hp-topbar">
          <div className="hp-brand">
            ...
            {showGameIcon && <img src="/assets/images/her_pher_v3/her_pher_v3.jpg" alt="Her Pher" className="hp-test-logo" />}
            {showGameName && <span className="hp-test-title">{t('home.games.herpher.title')}{t('common.version3')}</span>}
          </div>
