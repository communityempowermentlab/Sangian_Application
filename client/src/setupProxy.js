...
module.exports = function (app) {
    ...
    app.use((req, res, next) => {
        ...
        res.setHeader(
            ...
            "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com data:; img-src 'self' data: blob: http://localhost:*; connect-src 'self' ws://localhost:* ws://127.0.0.1:* http://localhost:* http://127.0.0.1:*; media-src 'self' data: blob: http://localhost:*; frame-src https://www.google.com https://accounts.google.com; object-src 'none';"
        );
