...
// @route   GET /api/admin/ankganit-v2
// @access  Private (admin)
const getFullConfig = async (req, res) => {
    try {
        const [categories] = await dbPool.query('SELECT * FROM ankganit_v2_categories ORDER BY display_order ASC');
        const [questions] = await dbPool.query('SELECT * FROM ankganit_v2_questions ORDER BY display_order ASC');
...
// @route   PUT /api/admin/ankganit-v2/categories/:id
// @access  Private (admin)
const updateCategory = async (req, res) => {
    try {
        ...
        await dbPool.query(
            'UPDATE ankganit_v2_categories SET name = ?, minimum_correct = ?, evaluation_type = ?, is_active = ? WHERE id = ?',
...
// @route   PUT /api/admin/ankganit-v2/questions/:id
// @access  Private (admin)
const updateQuestion = async (req, res) => {
    try {
        ...
        await dbPool.query(
            'UPDATE ankganit_v2_questions SET text = ?, title = ?, correct_answer = ?, remainder = ? WHERE id = ?',
